...
As ferramentas do jogador passam pelo `PlacementController`. A infecção é administrada pelo `InfectionSystem`; o combate pertence ao comportamento do policial e ao ciclo de vida centralizado no `EntityManager`. A HUD apenas emite intenções e exibe métricas.
...
- Civil: `WANDER`, `IDLE`, `INVESTIGATE`, `FLEE`, `INFECTED`, `TRANSFORMING`, `RESCUED`, `REMOVED`.
...
Backlog: tipos de vírus e zumbis, resgate/evacuação, médicos e exército, veículos, edifícios acessíveis, ruído, pânico, dia/noite, energia, editor de cenários, salvamento, estatísticas, replay e mods.
